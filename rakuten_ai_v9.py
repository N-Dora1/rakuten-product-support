...
1. click_score
クリックされやすさ。
商品名・特徴・用途が一目で理解でき、Xの投稿を見た人が「詳しく見たい」と感じるか。
単なる珍しさだけでは高得点にせず、購入検討につながるクリックを重視する。

2. interesting_score
興味・魅力度。
商品のメリットが明確で、「これ欲しい」「使ってみたい」と感じやすいか。
珍しいだけの商品より、実用性・悩み解決・ギフト需要など購入理由が明確な商品を高く評価する。

3. buy_score
購入転換力。
価格と商品の価値のバランス、用途の分かりやすさ、衝動購入のしやすさを評価する。
価格が高い場合は、それに見合う明確な価値があるかを厳しく見る。
「クリックはされそうだが買われにくい商品」は低く評価する。

4. trust_score
購入時の安心感。
レビュー平均とレビュー件数を重視する。
レビュー評価が高く、十分なレビュー件数がある商品を高く評価する。
レビューが極端に少ない、または評価が低い商品は慎重に採点する。

5. sns_score
SNS販売適性。
短い文章や画像だけでも商品の魅力・用途・購入理由を伝えやすいか。
Xで紹介したときに自然な投稿を作りやすく、楽天の商品ページへのクリックにつながりやすい商品を高く評価する。

5項目の合計をtotal_scoreとして100点満点にしてください。
特に「実際に購入される可能性」を重視し、単なる話題性や珍しさだけで高得点を付けないでください。
...
def main():
    ...
    # 投稿候補を保存
    save_path = "x_post_candidate.txt"
    with open(save_path, "w", encoding="utf-8") as f:
    print(f"\n投稿候補を保存しました：{save_path}")
        f.write(post)
    print("\n価格：", selected["price"], "円")
    print(
        "レビュー：",
